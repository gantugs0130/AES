...
unsigned char * AES::EncryptECB(unsigned char in[], unsigned int inLen, unsigned  char key[], unsigned int &outLen)
{
    ...
    unsigned char *alignIn  = PaddingNulls(in, inLen, blockBytesLen);
    unsigned char *out = new unsigned char[blockBytesLen];
    for (unsigned int i = 0; i < blockBytesLen; i+= blockBytesLen)
    {
...
unsigned int AES::GetPaddingLength(unsigned int len)
{
    ...
    cout << len % blockBytesLen<<"\n";
    if (len % blockBytesLen) {
        lengthWithPadding++;
    }
    lengthWithPadding *=  blockBytesLen;
    cout << lengthWithPadding << " end urt\n";
